...
class SyncPremierScrapper:
    ...
    def _get_usual_text(response) -> str:
        ...
        if 'v-list premium-adv-list' in search_area[0]:
            usual_text_bs = search_area[1]
        else:
            usual_text_bs = search_area[0]
        return str(usual_text_bs)

    @staticmethod
    def _form_urls(fresh_url_list: list) -> list:
        result = ['http://premier.ua'+ x for x in fresh_url_list]
        return result

    @staticmethod
    def find_link(text: str) -> str:
        pattern = re.compile(r'href=\"(/.+\.html)\"><img')
        attached_link_list = re.findall(pattern, str(text))
        attached_link = attached_link_list[0] if attached_link_list else None
    ...
    @staticmethod
    def _prepare(adv_text) -> str:
        res = '<tag>' + adv_text
        one_line_res = res.replace('\n', '')
        return one_line_res
    ...
    def _fetched_by_page_type(self, usual_text) -> list:
        adv_list_text = usual_text.split('<!-- c2 -->')[1:]
        adv_filtered_list = self.filter_data_off(adv_list_text)
        attached_links = [self.find_link(x) for x in adv_filtered_list]
        attached_links_1 = [x for x in attached_links if x is not None]
        fetched_links = self._form_urls(attached_links_1)
    ...
    def main_scrap(self):
        try:
            while True:
                ...
                self.parsed_items.extend(self._scrap(fetched_adv_links))
                print(self.parsed_items)
...
if __name__ == "__main__":
    ...
    a = SyncPremierScrapper(main_parse_url='https://premier.ua/zhilaia-nedvizhimost/uchastki-v-kharkove')
    b = a.main_scrap()
